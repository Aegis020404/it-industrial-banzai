 ...
 const MainTariffReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case 'TARIFF_TITLE_CHANGE': {
            return state.map((e,i)=>e.id == action.info.id ? {...e, title: action.info.text} : e)
        }
        case 'TARIFF_PRICE_CHANGE': {
            return state.map((e,i)=>e.id == action.info.id ? {...e, price: action.info.text} : e)
        }
        case 'TARIFF_DL_CHANGE': {
            return state.map((e,i)=>e.id == action.info.id ? {...e, dl: action.info.text} : e)
        }
        case 'TARIFF_DESCR_CHANGE': {
            return state.map((e,i)=>e.id == action.info.id ? {...e, descr: e.descr.map((el,n)=>n === action.info.count ? action.info.text : el)} : e)
        }
        case 'TARIFF_LOGO_CHANGE': {
            return state.map((e,i)=>e.id == action.info.id ? {...e, logo: action.info.text} : e)
        }
        default:
            return state
    }
