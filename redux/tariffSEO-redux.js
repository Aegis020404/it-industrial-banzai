...
export function tarrifSEORedux(state = inititalState, action) {
    switch (action.type) {
        ...
        case 'TITLE_TEXT_TARIFF_SEO_CHANGE': {
            return state.map(e=>e.id == action.info.id ? {...e, titleText: action.info.text} : e)
        }
        case 'COLOR_TARIFF_SEO_CHANGE': {
            return state.map(e=>e.id == action.info.id ? {...e, cases: e.cases.map(el=>el.id == action.info.count ? {...el, color: action.info.text} : el)} : e)
        }
        case 'BACKGROUND_TARIFF_SEO_CHANGE': {
            return state.map(e=>e.id == action.info.id ? {...e, cases: e.cases.map(el=>el.id == action.info.count ? {...el, background: action.info.text} : el)} : e)
        }
        case 'TITLE_ITEM_TARIFF_SEO_CHANGE': {
            return state.map(e=>e.id == action.info.id ? {...e, cases: e.cases.map(el=>el.id == action.info.count ? {...el, title: action.info.text} : el)} : e)
        }
        case 'PRICE_ITEM_TARIFF_SEO_CHANGE': {
            return state.map(e=>e.id == action.info.id ? {...e, cases: e.cases.map(el=>el.id == action.info.count ? {...el, price: action.info.text} : el)} : e)
        }
        case 'LIST_ITEM_TARIFF_SEO_CHANGE': {
            return state.map(e=>e.id == action.info.id ? {...e, cases: e.cases.map(el=>el.id == action.info.count ? {...el, list: el.list.map((str,i) => str.id == action.info.number ? {...str, text: action.info.text} : str)} : el)} : e)
        }
        case 'LIST_TEXT_TARIFF_SEO_CHANGE': {
            return state.map(e=>e.id == action.info.id ? {...e, cases: e.cases.map(el=>el.id == action.info.count ? {...el, list: action.info.text} : el )} : e)
        }
        default:
            return state
    }
