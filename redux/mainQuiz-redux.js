...
const initialState = [
    {id:1,activePage: 1,title:'Какой сайт вам нужен?',list:[{id:1,activeItem:0,subtitle: 'Лендинг', name: 'a', price: 80000},{id:2,activeItem:0,subtitle: 'Многостраничный', name: 'a', price: 150000},{id:3,activeItem:0,subtitle: 'Интернет-магазин', name: 'a', price: 200000},{id:4,activeItem:0,subtitle: 'Другое', name: 'a', price: 140000}]},
    {id:2,activePage: 0,title:'Какие задачи должен решать сайт?',list:[{id:1,activeItem:0,subtitle: 'Продавать товар/услугу', name: 'b', price: 15000},{id:2,activeItem:0,subtitle: 'Продавать несколько товаров/услуг', name: 'b', price: 35000},{id:3,activeItem:0,subtitle: 'Презентовать компанию', name: 'b', price: 25000},{id:4,activeItem:0,subtitle: 'Всё вместе/Другое', name: 'b', price: 50000}]},
    {id:3,activePage: 0, title:'Есть ли у вас старый сайт?', list:null, oldSiteUrl:'',oldSite: false, oldSiteText: 'Нет сайта'},
    {id:4,activePage: 0,title:'Есть ли у вас тексты для сайта?',list:[{id:1,activeItem:0,subtitle: 'Да', name: 'c', price: 15000},{id:2,activeItem:0,subtitle: 'Нет, нужно разработать сайт с нуля', name: 'c', price: 30000}]},
    {id:5,activePage: 0,list:'lastPage', lastTitle:'Примерная стоимость',lastDescr: 'Давайте обсудим детали проекта и рассчитаем точную стоимость?',lastHelper:'Нажимая на кнопку, вы даете согласие на обработку ваших персональных данных'}
]
...
const MainQuizReducer = (state = initialState,action)=>{
    switch (action.type) {
        case 'CHANGE_ACTIVE_PAGE_QUIZ': {
            
            const result =  state.map(e=>e.id == action.info.id ? {...e, activePage: 1} : {...e, activePage: 0})
        ...
        case 'RESET_ACTIVE_QUIZ': {
            const result =  state.map(e=>e.list!==null&&e.list!=='lastPage'? {...e, list:e.list.map(el=>{el.activeItem=false;return el})} :e.list==null?{...e, oldSiteUrl:'',oldSite: false,}:e).map(e=>e.id===1?{...e,activePage:1}:{...e,activePage:0})
            state.map((el)=>{
