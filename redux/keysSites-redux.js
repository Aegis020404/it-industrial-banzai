...
const keysSitesReducer = (state= initialState, action) => {
    switch (action.type) {
        ...
        case 'BACKGROUND_SITES_CHANGE': {
            const result = state.map(e=> e.id == action.info.id ? {...e, imageInfo: {...e.imageInfo, background: action.info.text}} : e)
            useFetchingPost(result.filter(e=>e.id==action.info.id)[0], 'keysSites', action.info.id)
            return result
        }
        case 'ALT_SITES_CHANGE': {
            const result = state.map(e=> e.id == action.info.id ? {...e, imageInfo: {...e.imageInfo, alt: action.info.text}} : e)
            useFetchingPost(result.filter(e=>e.id==action.info.id)[0], 'keysSites', action.info.id)
            return result
        }
        case 'IMG_SITES_CHANGE': {
            const result = state.map(e=> e.id == action.info.id ? {...e, imageInfo: {...e.imageInfo, img: action.info.text}} : e)
            useFetchingPost(result.filter(e=>e.id==action.info.id)[0], 'keysSites', action.info.id)
            return result
        }
